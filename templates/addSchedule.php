...
if ($workplace->getRule() == null) {
                ...
                $(function () {
                    $('#wp_day').datepicker({
                        onSelect: function (dateText) {
                            window.location = '<?= PluginEngine::getLink("WorkplaceAllocation", [], $admin ? "addSchedule" : "timetable") ?>' + '&day=' + dateText + '&wp_id=<?=$workplace->getId()?>&week=<?= Request::get('week') != null ? Request::get('week') : '0' ?>';
                        }
                    });
                })
            </script>
    ...
    $nowTime = new DateTime();
        ...
        if (Request::get('week') == null || Request::get('week') != '1') {
            ?>
        <a href="<?= PluginEngine::getLink('WorkplaceAllocation', ['wp_id' => $workplace->getId(), 'day' => $day->format('d.m.Y'), 'week' => 1], $admin ? 'addSchedule' : 'timetable') ?>">
            <?= Studip\Button::create('gesamte Woche zeigen') ?>
        </a>
        <?php
        } else {
            ?>
        <a href="<?= PluginEngine::getLink('WorkplaceAllocation', ['wp_id' => $workplace->getId(), 'day' => $day->format('d.m.Y')], $admin ? 'addSchedule' : 'timetable') ?>">
        ...
        if ($admin):
    ?>
        <form action="<?=PluginEngine::getLink('WorkplaceAllocation', ['wp_id' => $workplace->getId(), 'day' => $day->format('d.m.Y')], 'addSchedule')?>" method="post">
            ...
            <?= Studip\Button::create('Ausgewählten Tag blocken', null, ['type' => 'submit'])?>
        </form>
    <?php
    ...
    <div class="timetable">
        ...
        foreach ($days as $day) {
            ...
            for ($time = clone $tableStartTime; $time < $tableEndTime; $time->add($slotDuration)) {
                ...
                if ($foundSchedule != null) {
                    ...
                    if ($diff != 0) {
                        ...
                        if ($admin) {
                            ...
                            $fillButton = \Studip\Button::create(_('Nachfolgende Termine aufrücken'), null, ['type' => 'submit', 'class' => 'schedule', 'style' => 'max-height: ' . $diffHeight . 'rem; min-height: ' . $diffHeight . 'rem;']);

                            $linkToAddSchedule = PluginEngine::getLink('WorkplaceAllocation', ['wp_id' => $workplace->getId(), 'day' => $day->format('d.m.Y')], $admin ? 'addSchedule' : 'timetable');
                    ...
                    if (!$foundSchedule->isBlocked() || $admin) {
                        ...
                        if ($admin || $foundSchedule->getOwner()->user_id == get_userid()) {
                            ...
                            print('<a href="' . PluginEngine::getLink('WorkplaceAllocation', ['wp_id' => $workplace->getId(), 's_id' => $foundSchedule->getId()], 'editSchedule') . '" title="Termin bearbeiten">' . (new Icon('edit'))->asImg() . '</a>');

                            if ($foundSchedule->getStart() > new DateTime()) {
                                ?>
                                    <form action="<?= PluginEngine::getLink('WorkplaceAllocation', [], 'removeSchedule') ?>" method="post" style="display:inline">
                            ...
                            if ($admin) {
                                ?>
                                    <form action="<?= PluginEngine::getLink('WorkplaceAllocation', [], 'manageBlacklist') ?>" method="post" style="display:inline">
                ...
                } elseif ($workplace->getRule()->hasPause() && $endTime > $pauseStartTime && $startTime < $pauseEndTime) {
                ...
                } elseif ($startTime >= $nowTime && $workplace->getRule()->isDayBookable($day, $admin) && $endTime <= $tableEndTime) {
                    /** @var \Studip\Button $bookButton */
                    $bookButton = \Studip\Button::create(_($startTime->format('H:i') . ' Uhr bis ' . $endTime->format('H:i') . ' Uhr buchen'), null, ['type' => 'submit', 'class' => 'schedule', 'style' => 'max-height: ' . $height . 'rem; min-height: ' . $height . 'rem;']);

                    print('<form action="' . PluginEngine::getLink('WorkplaceAllocation', ['wp_id' => $workplace->getId(), 'week' => '1', 'day' => $day->format('d.m.Y')], $admin ? 'addSchedule' : 'timetable') . '" method="post">');
