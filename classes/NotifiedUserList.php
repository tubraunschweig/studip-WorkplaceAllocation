...
class NotifiedUserList implements IteratorAggregate
{
    ...
    public function list_size() {
        $data = DBManager::get()->fetchAll("SELECT * FROM wp_notified_user_list WHERE context_id = ?",
    ...
    public function isOnList($user_id) {
        $data = DBManager::get()->fetchAll("SELECT * FROM wp_notified_user_list WHERE user_id = ? AND context_id = ?",
            array($user_id, $this->context_id));
