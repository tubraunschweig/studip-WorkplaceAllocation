...
class WorkplaceAllocation extends StudIPPlugin implements StandardPlugin, HomepagePlugin
{
    ...
    public function __construct()
    {
        ...
        if (!isset($request['username']) || $request['username'] == $currentUser->username) {
            ...
            $mySchedules = new Navigation('Arbeitsplätze', PluginEngine::getURL('WorkplaceAllocation', [], 'my_schedules'));
    ...
    public function getTabNavigation($course_id)
    {
        ...
        $workplaceAllocation = new Navigation('Arbeitsplätze', PluginEngine::getURL('WorkplaceAllocation', [], 'show'));
        ...
        if ($this->user_has_admin_perm($course_id)) {
            $workplacesAdminNav = new Navigation('Arbeitsplätze', PluginEngine::getURL('WorkplaceAllocation', [], 'admin'));
    ...
    public function admin_action()
    {
        ...
        $actionsWidget->addLink(
            ...
            PluginEngine::getLink('WorkplaceAllocation', [], 'addWorkplace'),
            new Icon('add')
        );
        $actionsWidget->addLink(
            'Sperrungen verwalten',
            PluginEngine::getLink('WorkplaceAllocation', [], 'manageBlacklist'),
            new Icon('community')
        );
        $actionsWidget->addLink(
            'Benachrichtigungstexte verwalten',
            PluginEngine::getLink('WorkplaceAllocation', [], 'manageMail'),
            new Icon('mail')
        );
        $actionsWidget->addLink(
            'Mailingliste verwalten',
            PluginEngine::getLink('WorkplaceAllocation', [], 'manageNotifiedUsers'),
            new Icon('mail')
        );
        $actionsWidget->addLink(
            'Alle Arbeitsplätze drucken',
            PluginEngine::getLink('WorkplaceAllocation', [], 'pdf'),
            new Icon('print'),
            ['target' => '_blank']
        );
    ...
    public function saveActivation_action()
    {
        ...
        header('Location: ' . PluginEngine::getLink('WorkplaceAllocation', [], 'admin'));
        exit;
    }
        ...
        if (Request::isPost()) {
            ...
            if (isset($request['day']) && (empty($request['daily_start_hour']) || empty($request['daily_start_minute']) || empty($request['daily_end_hour']) || empty($request['daily_end_minute']))) {
                ...
                $messageBoxes[] = MessageBox::success(
                    ...
                    ['<a href="' . PluginEngine::getLink('WorkplaceAllocation', [], 'admin') . '">Zurück zur Übersicht</a>']
                );
            }
        }
    ...
    public function addSchedule_action($isSetNavigation = false)
    {
        ...
        $lock = new FileLock('wp_'.Request::get('wp_id'));
        ...
        if (Request::isPost() && !(Request::get('embedded', $default = false))) {
            ...
            if ($lock->isLocked($data)) {
                if ($data['timestamp'] + 60 > time()) {
                    $lock->release();
                } else {
                    $messageBoxes[] = MessageBox::error('Die Änderung konnte nicht durchgeführt werden');
                    $success = false;
                }
            }
            ...
            if (!$lock->isLocked($data)) {
                ...
                if (isset($request['next_schedule']) && $request['next_schedule'] == 'true') {
                    if (!$workplace->getRule()->bookFirstPossibleSchedule($workplace, $day, $admin)) {
                        if (!$workplace->getRule()->isDayBookable($day, $admin, $workplace)) {
                            $messageBox = MessageBox::error('Der Termin konnte nicht gebucht werden, dies kann verschiedene Ursachen haben', ['Sie wurden gesperrt.', 'Es ist nur ein Termin pro Nutzer und Tag zugelassen.', 'Es ist zu einer Kollision gekommen, in diesem Falle versuche in der Übersicht nochmal einen Termin für diesen Tag zu buchen um einen Platz auf der Warteliste zu bekommen.']);
                        } else {
                ...
                } elseif (isset($request['action']) && $request['action'] == 'move_up' && isset($request['wp_schedule_id']) && isset($request['wp_schedule_new_start'])) {
                ...
                } elseif (isset($request['dont_book'])) {
                    ...
                    if ($workplace->getRule()->isBookable(new DateTime('@' . $start), new DateInterval($duration), $workplace, $admin)) {
                        ...
                        $messageBox = MessageBox::error('Der Termin konnte nicht gebucht werden, dies kann verschiedene Ursachen haben', ['Der Termin ist bereits belegt.', 'Es ist nur ein Termin pro Nutzer und Tag zugelassen.']);
                    }
                }
    ...
    public function editSchedule_action()
    {
        ...
        // file based lock to limit schedule manipulations to one user per workplace at once
        FileLock::setDirectory($GLOBALS['TMP_PATH']);
        $lock = new FileLock('wp_'.$schedule->getWorkplace()->getId());
            ...
            if (!$lock->isLocked($data)) {
                ...
                if ($success) {
                    $backLink = PluginEngine::getLink(
                        'WorkplaceAllocation',
                        ['wp_id' => $schedule->getWorkplace()->getId(),
                              'week' => '1',
                              'day' => $schedule->getStart()->format('d.m.Y')],
                        $admin ? 'addSchedule' : 'timetable'
                    );

                    $messageBoxes[] = MessageBox::success('Erfolgreich gespeichert', ['<a href="' . $backLink . '">zurück</a>']);
                }
    ...
    public function removeSchedule_action()
    {
        ...
        if (Request::isPost()) {
            ...
            header('Location: ' . PluginEngine::getURL('WorkplaceAllocation', ['wp_id' => $schedule->getWorkplace()->getId(), 'week' => '1', 'day' => $schedule->getStart()->format('d.m.Y')], $admin ? 'addSchedule': 'timetable'));
        }
    }
    ...
    public function manageMail_action()
    {
        ...
        foreach ($defaultMessageTexts as $messageTextId => $messageTextDetails) {
            $studipMessage = WpMessages::findBySQL("context_id = ? AND hook_point = ?", [Request::get('cid'), $messageTextId]);
    ...
    public function editMailtext_action()
    {
        ...
        $message = WpMessages::findBySQL("context_id = ? AND hook_point = ?", [Request::get('cid'), Request::get('hook_point')]);
        ...
        if (Request::isPost()) {
            ...
            header('Location: ' . PluginEngine::getLink('WorkplaceAllocation', [], 'manageMail'));
        }
    ...
    */
        ...
        if (Request::isPost()) {
            ...
            switch ($request['action']) {
                ...
                case 'delete':
                    ...
                    header('Location: ' . PluginEngine::getLink('WorkplaceAllocation', [], 'manageNotifiedUsers'));
                    break;
                default:
                    break;
            }
    ...
    public function pdf_action()
    {
        ...
        foreach ($workplaces as $wp) {
            ...
            $pdf->Rect(80, 35, 122, 0.5, 'F', [], [190, 30, 60]);
            ...
            for ($i = 0; $i < $steps; $i += (60*30)) {
                $pdf->Rect(27, $topStart + $i * $stepHeight, 175, 0.5, 'F', [], [200, 200, 200]);
            ...
            foreach ($schedules as $schedule) {
                ...
                $pdf->Rect(47, $topStart + $scheduleStartTableStart->getTimestamp() * $stepHeight, 155, $scheduleDurationTime->getTimestamp() * $stepHeight, 'FD', ['all' => ['width' => 0.5, 'color' => [35, 64, 153]]], [255,255,255]);
